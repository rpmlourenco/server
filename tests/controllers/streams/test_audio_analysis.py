...
async def test_background_streaming_uses_pcm_decoder(codec: ContentType, bit_depth: int) -> None:
    ...
    controller.mass.streams.audio.get_media_stream = stream  # type: ignore[method-assign]

    await controller._run_background_streaming_inner(
        streamdetails.uri, streamdetails, cast("list[AudioAnalysisProvider]", providers)
    )
